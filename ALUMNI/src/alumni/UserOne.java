...
public class UserOne implements ActionListener, Runnable //userone cls implements both the ActionListener and Runnable interfaces. 
{
    ...
    UserOne() {
        ...
        JLabel status = new JLabel("ADMIN"); //text of apshah
        ...
        try {
            Socket socket = new Socket("localhost", 2003); //  connects to a server running on the same machine  at port 2003. 
            writer = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream())); //used to send data to the server.
            reader = new BufferedReader(new InputStreamReader(socket.getInputStream())); //it's used to receive data from the server.
        } catch (Exception e) {
            e.printStackTrace();
        }
